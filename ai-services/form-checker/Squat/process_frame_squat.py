...
class ProcessFrame:
    ...
    def _get_state(self, knee_angle):
        knee = None
        curr = self.state_tracker.get('curr_state')

        # Add hysteresis: require a buffer to transition OUT of current state
        if curr == 's1':
            # Must go deeper than TRANS[0] + buffer to leave s1
            if knee_angle > self.thresholds['HIP_KNEE_VERT']['TRANS'][0] + 5:
                knee = 2
            else:
                knee = 1
        elif curr == 's2':
            # Stay in s2 unless clearly back in s1 or forward into s3
            if knee_angle < self.thresholds['HIP_KNEE_VERT']['NORMAL'][1] - 5:
                knee = 1
            elif knee_angle > self.thresholds['HIP_KNEE_VERT']['PASS'][0] - 5:
                knee = 3
            else:
                knee = 2
        elif curr == 's3':
            if knee_angle < self.thresholds['HIP_KNEE_VERT']['TRANS'][1] - 5:
                knee = 2
            else:
                knee = 3
        else:
            # Initial state — use original logic
