...
class ProcessFrameShoulderPress:
    def __init__(self, thresholds, flip_frame=False):
        ...
        self.state_tracker = {
            ...
            # 0 --> Elbows too low, 1 --> Arms not in sync, 2 --> Press higher, 3 --> Overextending
        ...
        self.FEEDBACK_ID_MAP = {
                                0: ('ELBOWS TOO LOW',    215, (0, 153, 255)),
                                1: ('ARMS NOT IN SYNC',  215, (0, 153, 255)),
                                2: ('PRESS HIGHER',      170, (255, 80, 80)),
                                3: ('OVEREXTENDING',     125, (255, 80, 80))
                               }


    def _get_state(self, elbow_angle):
        # Angle goes DOWN as arms press UP:
        # s1 = arms down  → HIGH angle (45-65°)
        # s2 = mid press  → MID angle  (25-44°)
        # s3 = arms up    → LOW angle  (5-24°)
        elbow = None
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, press_higher):
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Shoulder width check: large when facing forward, small when sideways.
            shoulder_width = abs(int(left_shldr_coord[0]) - int(right_shldr_coord[0]))

            if shoulder_width < self.thresholds['OFFSET_THRESH']:
                ...
                if self.state_tracker['INACTIVE_TIME_FRONT'] >= self.thresholds['INACTIVE_THRESH']:
                    self.state_tracker['PRESS_COUNT']    = 0
                    self.state_tracker['IMPROPER_PRESS'] = 0
            ...
            # Person is facing the camera — process normally.
            else:
                ...
                # ------------------- Angle calculations -------------------
                left_elbow_angle  = find_angle(left_shldr_coord,  left_elbow_coord,  left_wrist_coord)
                right_elbow_angle = find_angle(right_shldr_coord, right_elbow_coord, right_wrist_coord)
                ...
                avg_elbow_angle = int((left_elbow_angle + right_elbow_angle) / 2)
                elbow_diff      = abs(int(left_elbow_angle) - int(right_elbow_angle))
                # -----------------------------------------------------------

                # Draw arcs at elbows
                cv2.ellipse(frame, left_elbow_coord,  (20, 20), angle=0, startAngle=0, endAngle=left_elbow_angle,
                            color=self.COLORS['white'], thickness=3, lineType=self.linetype)
                cv2.ellipse(frame, right_elbow_coord, (20, 20), angle=0, startAngle=0, endAngle=right_elbow_angle,
                            color=self.COLORS['white'], thickness=3, lineType=self.linetype)
                ...
                cv2.line(frame, right_elbow_coord, right_wrist_coord, self.COLORS['light_blue'], 4, lineType=self.linetype)
                cv2.line(frame, left_shldr_coord,  right_shldr_coord, self.COLORS['light_blue'], 4, lineType=self.linetype)
                ...
                current_state = self._get_state(avg_elbow_angle)
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        self.state_tracker['PRESS_COUNT'] += 1
                        play_sound = str(self.state_tracker['PRESS_COUNT'])
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    # Elbows too low
                    if left_shoulder_angle < self.thresholds['SHOULDER_THRESH'][0] or \
                       right_shoulder_angle < self.thresholds['SHOULDER_THRESH'][0]:
                        self.state_tracker['DISPLAY_TEXT'][0] = True

                    # Arms not in sync
                    if elbow_diff > self.thresholds['SYNC_THRESH']:
                        self.state_tracker['DISPLAY_TEXT'][1] = True
                        self.state_tracker['INCORRECT_POSTURE'] = True

                    # Mid-press but not reaching the top — prompt to press higher
                    # ELBOW_THRESH[0] is mid zone, ELBOW_THRESH[1] is top zone entry
                    if self.thresholds['ELBOW_VERT']['TRANS'][0] <= avg_elbow_angle <= self.thresholds['ELBOW_VERT']['TRANS'][1] and \
                       self.state_tracker['state_seq'].count('s2') == 1:
                        self.state_tracker['PRESS_HIGHER'] = True

                    # Overextending — angle went below safe floor
                    elif avg_elbow_angle < self.thresholds['ELBOW_VERT']['PASS'][0]:
                ...
                left_elbow_text_x  = left_elbow_coord[0]  - 60
                right_elbow_text_x = right_elbow_coord[0] + 10

                if self.flip_frame:
                    frame = cv2.flip(frame, 1)
                    left_elbow_text_x  = frame_width - left_elbow_coord[0]  + 10
                    right_elbow_text_x = frame_width - right_elbow_coord[0] - 60

                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['PRESS_HIGHER'] = False

                self.state_tracker['COUNT_FRAMES'][self.state_tracker['DISPLAY_TEXT']] += 1

                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['PRESS_HIGHER'])
                ...
                cv2.putText(frame, str(int(left_elbow_angle)),  (left_elbow_text_x,  left_elbow_coord[1]  + 10), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(right_elbow_angle)), (right_elbow_text_x, right_elbow_coord[1] + 10), self.font, 0.6, self.COLORS['light_green'], 2, lineType=self.linetype)
