...
def ensure_pose_model():
    ...
    if os.path.exists(MODEL_PATH) and os.path.getsize(MODEL_PATH) > 1000:
        return MODEL_PATH
    print(f"[form-checker] Downloading pose model to {MODEL_PATH}…")
...
class PoseLandmarkerAdapter:
    """MediaPipe Tasks API adapter matching legacy mp.solutions.pose.Pose interface."""
    ...
    def __init__(
        ...
        del static_image_mode, model_complexity, smooth_landmarks
    ...
    def process(self, frame_rgb):
        ...
        landmarks = [
            _Landmark(lm.x, lm.y, lm.z, lm.visibility)
            for lm in result.pose_landmarks[0]
        ]
        return _PoseResults(landmarks)
